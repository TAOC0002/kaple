...
def convert_examples_to_features_trex(examples, label_list, max_seq_length,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        features.append(
            rcInputFeatures(input_ids=input_ids,
                                input_mask=input_mask,
                                segment_ids=segment_ids,
                                label_id=label_id,
...
class DDIProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, dataset_type, negative_sample):
        ...
        for (i, line) in enumerate(lines):
            ...
            text_b = line[0]
            label = line[2]
            if label == 'DDI-false' and dataset_type == 'train':
                no_relation_number -= 1
                if no_relation_number > 0:
                    examples.append(
                        InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label))
...
def convert_examples_to_features_ddi(examples, label_list, max_seq_length,
                                      tokenizer, output_mode,
                                      cls_token_at_end=False,
                                      cls_token='[CLS]',
                                      cls_token_segment_id=1,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # Roberta tokenizer add 'Ġ' in front of the tokens if they arre preceded by a whitespace
        try:
            subj_special_start = tokens.index('Ġ@')
        except:
            continue
        try:
            obj_special_start = tokens.index('Ġ@', subj_special_start+1, -1)
        except:
            continue
        ...
        features.append(
            rcInputFeatures(input_ids=input_ids,
                                ...
                                subj_special_start_id=subj_special_start_id,
                                obj_special_start_id=obj_special_start_id))
    logger.info("Number of examples: {}".format(len(features)))
    return features
