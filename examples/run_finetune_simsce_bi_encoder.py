...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for example_index, example in enumerate(examples):
        ...
        if mode == "cross":
            ...
            prior_input_ids, prior_input_mask, prior_segment_ids = tokens_to_ids(tokenizer, prior, max_seq_length)
            claim_input_ids, claim_input_mask, claim_segment_ids = tokens_to_ids(tokenizer, claim, max_seq_length)
            label = int(example.label) if example.label is not None else None
    ...
    if args.do_train:
        ...
        for step in bar:
            if args.mode == "cross":
                ...
                prior_input_ids, prior_input_mask, prior_segment_ids, prior_label_ids = prior_batch
                claim_input_ids, claim_input_mask, claim_segment_ids, claim_label_ids = claim_batch
                assert prior_label_ids == claim_label_ids
                ...
                prior_output_logits = prior_output_logits.detach().cpu().numpy()
                ...
                claim_output_logits = claim_output_logits.detach().cpu().numpy()

                loss_fct = BCELoss()
                cosine_similarity = 
                logits = self.classifier(sequence_output[:, 0, :].squeeze(dim=1))
                reshaped_logits = logits.view(-1, self.num_labels)
                loss = loss_fct(sigmoid(reshaped_logits).squeeze(dim=1), prior_label_ids)
