...
from util import read_sts_examples, convert_examples_to_features
...
def run_eval(eval_data, pretrained_model, patent_model, logger, args, device, global_step, tb_writer, train_loss, batch_size, mode, _cycle):
    ...
    if mode == "cross":
        ...
        for prior_input_ids, prior_input_mask, prior_segment_ids, label_ids in eval_prior_dataloader:
            ...
            with torch.no_grad():
                ...
                fac_cosines = args.score_range*sigmoid(fac_cosines)
    ...
    return eval_spearmanr, eval_pearsonr
...
def main():
    ...
    train_features_bi = convert_examples_to_features(train_examples, tokenizer, args.max_seq_length, "bi", logger, is_training=True, label_type='regression')
    logger.info("Preprocessing training data for cross-encoder")
    train_features_cross = convert_examples_to_features(train_examples, tokenizer, args.max_seq_length, "cross", logger, is_training=True, label_type='regression')

    eval_examples = read_sts_examples(os.path.join(args.data_dir, args.year + '.val.tsv'), is_training=True)
    logger.info("Preprocessing training data for bi-encoder")
    ...
    bicheckpoints = ['../proc_data/roberta_sts/june10-our-orig-cross/']
    ...
    for _cycle in range(1, args.cycles+1):
        ...
        for step in bar:
            ...
            if (global_step + 1) % args.eval_steps == 0 and eval_flag:
                ...
                pretrained_model.eval()
                patent_model.eval()
                eval_spearmanr, eval_pearsonr = run_eval(eval_data_cross, pretrained_model, patent_model, logger, args, device, global_step, tb_writer, train_loss, batch_size, "cross", _cycle)
        ...
        for step in bar:
            ...
            loss = loss_fct(args.score_range*sigmoid(cosines), labels) + loss_fct(args.score_range*sigmoid(fac_cosines), labels)
            ...
            if (global_step + 1) % args.eval_steps == 0 and eval_flag:
                ...
                pretrained_model.eval()
                patent_model.eval()
                eval_spearmanr, eval_pearsonr = run_eval(eval_data_bi, pretrained_model, patent_model, logger, args, device, global_step, tb_writer, train_loss, batch_size, "bi", _cycle)
                if args.metrics == 'spearmanr':
                    if eval_spearmanr > best_spearmanr:
                        best_spearmanr = eval_spearmanr
                        eval_flag = True
