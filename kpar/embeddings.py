...
def get_embedding(examples, pretrained_model, patent_model):
    first = True
    for e in examples:
        input_ids = torch.tensor([e.input_ids], dtype=torch.long).unsqueeze(0)
        input_mask = torch.tensor([e.input_mask], dtype=torch.long).unsqueeze(0)
        segment_ids = torch.tensor([e.segment_ids], dtype=torch.long).unsqueeze(0)
        model_outputs = pretrained_model(input_ids=input_ids, token_type_ids=input_mask, attention_mask=segment_ids)
